...
public:
    ...
    // i, j는 픽셀 좌표, offsetX, offsetY는 [0, 1) 범위의 값
    Ray getRay(int i, int j, float offsetX, float offsetY) const
    {
        // offsetX, offsetY는 보통 [0, 1) 범위의 값
        float u_ = l + (r - l) * ((i + offsetX) / (float)Nx);
        float v_ = b + (t - b) * ((j + offsetY) / (float)Ny);
        glm::vec3 screenPos = eye - d * W + u_ * U + v_ * V;
        glm::vec3 dir = glm::normalize(screenPos - eye);
        return Ray(eye, dir);
    }
